...
import time
import logging
from PySide6.QtWidgets import QApplication, QDialog, QVBoxLayout, QHBoxLayout, QLabel, QLineEdit, QPushButton, QFormLayout, QGroupBox, QSpinBox, QFrame, QMessageBox
...
def _refresh_styles(*widgets):
    """Refresh widget styles in one polish pass."""
    for w in widgets:
        w.style().unpolish(w)
        w.style().polish(w)

def validate_ip_address(ip_address, port, timeout=1):
    """Validate an IP address with a short TCP connection attempt."""
    try:
...
class AddCameraDialog(QDialog):
    def __init__(self, parent=None, camera_data=None):
        """Create a dialog for adding or editing a camera."""
    ...
    def validate_and_accept(self):
        """Validate input and continue to ROI selection."""
        ...
        _refresh_styles(self.camera_name, self.camera_ip, self.stream_path)
        ...
        if not camera_name:
            ...
            _refresh_styles(self.camera_name)
            return
    ...
    def proceed_to_roi_selection(self):
        """Open ROI selection after validation succeeds."""
        wait_dialog = QMessageBox(self)
