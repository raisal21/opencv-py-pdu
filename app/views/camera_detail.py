...
import logging
import gc
from PySide6.QtWidgets import QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, QLabel, QComboBox, QPushButton, QFrame, QSizePolicy, QSpacerItem, QMessageBox
from PySide6.QtCore import Qt, QThread, QSize, QTimer, QDateTime, QPointF, QMargins, Slot, QUrl, QThreadPool
from PySide6.QtGui import QPainter, QColor, QBrush, QPen, QFont, QIcon, QDesktopServices
...
class CoverageStatusWidget(QFrame):
    ...
    def updateStatus(self, percentage):
        ...
        if percentage >= 95:
            self.indicator.setStyleSheet("background-color: #F44336; border-radius: 15px;")
            self.setStyleSheet("background-color: rgba(244, 67, 54, 0.1);")
        elif percentage >= 85:
            self.indicator.setStyleSheet("background-color: #FFEB3B; border-radius: 15px;")
            self.setStyleSheet("background-color: rgba(255, 235, 59, 0.1);")
        else:
            self.indicator.setStyleSheet("background-color: #4CAF50; border-radius: 15px;")
...
class CameraDetailUI(QMainWindow):
    ...
    def on_processed_frame(self, display_frame, metrics):
        """Update UI state from a processed frame."""
        if self._is_closing:
            return
    ...
    def _init_camera_new(self):
        """Start the stream worker for the current camera."""
    ...
    def closeEvent(self, event):
        """Shut down timers and worker threads before closing."""
        if self._is_closing:
            event.ignore()
            return
